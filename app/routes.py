...
from app.models import User, UserKeyHistory, Thread, ThreadParticipant, Message
import base64
import os
import bleach
...
def login():
    if request.method == 'POST':
        ...
        elif crypto.verify_user(user.public_key, signature, username.encode()):
...
def thread(thread_id):
    ...
    for msg in messages:
        try:
            shared_secret = crypto.decapsulate_key(base64.b64decode(msg.ciphertext),
    ...
    for user in participants:
        ...
        if file:
            ...
            file_path = os.path.join('app/uploads',
                                     f"{thread_id}_{user.id}_{hashlib.sha3_256(file_name.encode()).hexdigest()}")
    ...
    for user in participants:
        try:
            ...
            decrypted_content = crypto.decrypt_data(encrypted_content,
                                                    shared_secret).decode() if encrypted_content else ""
            emit('new_message', {
                'thread_id': thread_id,
                'sender': session['username'],
